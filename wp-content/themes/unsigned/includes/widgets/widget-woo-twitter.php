...
class Woo_Twitter extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $title ) {
			...
			?><h3 class="tlogo"><img src="<?php echo esc_url( trailingslashit( get_template_directory_uri() ) . 'images/twitter.png' ); ?>" alt="Twitter" /><?php _e( 'Twitter', 'woothemes' ); ?></h3><?php
		}
		?><div class="back"><ul id="twitter_update_list_<?php echo $unique_id; ?>"><li></li></ul>
			<p><?php _e( 'Follow', 'woothemes' ); ?> <a href="http<?php ( is_ssl() ? 's' : '' ) ?>://twitter.com/<?php echo esc_attr( $username ); ?>"><strong>@<?php echo $username; ?></strong></a> <?php _e( 'on Twitter', 'woothemes' ); ?></p></div><div class="clear"></div><?php
